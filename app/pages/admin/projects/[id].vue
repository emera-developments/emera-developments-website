<template>
  <div class="max-w-3xl">
    ...
    <div v-if="pending" class="space-y-4">
      <div v-for="i in 5" :key="i" class="h-32 bg-gray-200 rounded-2xl animate-pulse" />
    </div>

    <div v-else-if="!project" class="text-center py-20 text-gray-400">
      <p>Project not found.</p>
      <NuxtLink to="/admin/projects" class="text-gold-500 hover:underline text-sm">← Back to projects</NuxtLink>
    </div>
    ...
    <form v-else class="space-y-8" @submit.prevent="handleSubmit">
      ...
      <p v-if="saved" class="text-sm text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3">Changes saved successfully.</p>
...
const specsEn = reactive<{ key: string; value: string }[]>([])
const specsAr = reactive<{ key: string; value: string }[]>([])

function objectToSpecs(obj: Record<string, string>) {
...
watch(project, (p) => {
  ...
  specsEn.splice(0, specsEn.length, ...objectToSpecs(p.specifications ?? {}))
  specsAr.splice(0, specsAr.length, ...objectToSpecs(p.specifications_ar ?? {}))
  if (!specsEn.length) specsEn.push({ key: '', value: '' })
  if (!specsAr.length) specsAr.push({ key: '', value: '' })
}, { immediate: true })
